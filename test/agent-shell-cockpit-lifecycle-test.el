  ...
  (cockpit-test-with-worktree
   ...
   (let ((archive (agent-shell-cockpit-workspace-archive workspace))
     ...
     (cl-letf (((symbol-function 'agent-shell-cockpit-git--run)
       ...
       (let ((partial (agent-shell-cockpit-workspace-restore archive)))
         (should (equal (map-elt partial 'state) "active"))
         (should (seq-some (lambda (entry) (map-elt entry 'restoreError))
                           (map-elt partial 'worktrees)))))
